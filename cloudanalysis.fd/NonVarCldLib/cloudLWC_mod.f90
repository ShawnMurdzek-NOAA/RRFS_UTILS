...
! calculate RH later at single point, don't need a array
!        if(cloudqvis(i,j,k) > 1.e-5) then
...
!       3K/100mb below (nearer sfc)
            rhijk=0.0_r_single
            if(cloudqvis(i,j,k) > 1.e-5_r_kind) rhijk=q_bk(i,j,k)/cloudqvis(i,j,k)
...
!               write(*,'(a,3i4,f8.3)') 'skip building cloud in stable layer',i,j,k,stab*10000.0
            ...
            elseif(rhijk < 0.40 .and. ((cloudqvis(i,j,k)-q_bk(i,j,k)) > 0.003_r_kind)) then
!               write(*,'(a,3i4,2f6.2)') 'skip building cloud in too-dry layer',i,j,k,& 
!                            rhijk,(cloudqvis(i,j,k)-q_bk(i,j,k))*1000.0
