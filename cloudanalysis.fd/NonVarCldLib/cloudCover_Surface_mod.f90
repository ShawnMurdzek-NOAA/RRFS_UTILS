...
!
   IF(.false.) THEN     ! Set inversion strength flag
     DO j = 2,nlat-1
       DO i = 2,nlon-1
       call BckgrndCC(nsig,    &
                 t_bk(i,j,:),p_bk(i,j,:),q(i,j,:),h_bk(i,j,:),zh(i,j),  &
                 cv_bk(:),tbk_k(:))    ! out

         if_cloud_exist=.false.
         do k=nsig-1,2,-1
           if(cld_cover_3d(i,j,k) > 0.01_r_kind) then
             cf_model_base = cv_bk(k)
             t_model_base = tbk_k(k)
             ht_base=h_bk(i,j,k)
             if_cloud_exist=.true.
           endif
         enddo
         ...
         if(if_cloud_exist) then
           do k=2, nsig-1
             if(cld_cover_3d(i,j,k) > 0.01_r_kind) then
               l_cf=.false.
               l_inversion=.false.
               t_dry_adiabat = tbk_k(2) -.0098_r_kind * (h_bk(i,j,k) - h_bk(i,j,2))
               t_inversion_strength = tbk_k(k) - t_dry_adiabat
 
               IF( (tbk_k(k) > t_model_base)  .and.  &
                   (tbk_k(k) > 283.15_r_kind) .and.  &   !   temp check
                   (t_inversion_strength > 4._r_kind) ) then ! delta theta chk
                       l_inversion = .true.           ! Inversion exists
               endif
               IF( (cv_bk(k) < cf_model_base - 0.3_r_kind) .and. &
